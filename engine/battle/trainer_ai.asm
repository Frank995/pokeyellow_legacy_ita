...
.loopFindMinimumEntries_backupfirst	; make a backup of the scores
...
.done
;;;;;;;;;; clear these values at the end of an AI cycle, they only apply when the player has switched or healed in a turn
	xor a
...
; CHANGED: AKA the "Dont do stupid things no player would ever do" AI subroutine, many new default AI restrictions added
...
;;;;;;;;;; ADDED: function for checking if the player can have leech seed applied and whether they already have it applied
CheckSeeded:
	push hl
...
;;;;;;;;;; ADDED: function for checking if the player's pokemon is unaffected by specific status moves.
...
;;;;;;;;;; ADDED: function that allows AI to avoid OHKO moves if they will never do anything to the player's pokemon due to speed differences
...
; CHANGED: AKA the "Boost stats on the first turn" subroutine
; slightly encourage moves with specific effects on the first turn. (FIXED: used to be the second turn, made it first turn)
...
; CHANGED: AKA the "Use Effective damaging moves offensively" subroutine
...
; FIXED: this subroutine won't cause the AI to prefer status moves 
...
;;;;;;;;;; ADDED: function that allows AI to be aware if they are slower than the opponent. Allows them to prefer priority moves.
CompareSpeed:
	push hl
	push de
	push bc
...
; ADDED: if the opponent has less than 1/2 health they will prefer healing moves if they use AI subroutine 3
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ; FIXED: AI should not use actions (items / switching) if in a move that prevents such a thing
...
AICureStatus:	; CHANGED: modified to be more robust and also undo stat changes of brn/par
...
; ADDED: if enemy HP is below a 1/[wUnusedC000], store 1 in wUnusedC000.
...
; ADDED: these wram properties are used to make sure the 
...
; ADDED: doubles attack if burned or quadruples speed if paralyzed.
